...
const configSchema = z.object({
	SERVER_PORT: z
		...
		.default("8080"),
	SERVER_READ_TIMEOUT: z
		.string()
		.transform((val) => parseInt(val, 10))
		.default("30"),
	SERVER_WRITE_TIMEOUT: z
		.string()
		.transform((val) => parseInt(val, 10))
		.default("30"),
	SERVER_IDLE_TIMEOUT: z
		.string()
		.transform((val) => parseInt(val, 10))
		.default("60"),
	...
	DATABASE_MAX_OPEN_CONNS: z
		.string()
		.transform((val) => parseInt(val, 10))
		.default("25"),
	DATABASE_MAX_IDLE_CONNS: z
		.string()
		.transform((val) => parseInt(val, 10))
		.default("25"),
	DATABASE_CONN_MAX_LIFETIME: z
		.string()
		.transform((val) => parseInt(val, 10))
		.default("300"),
	DATABASE_CONN_MAX_IDLE_TIME: z
		.string()
		.transform((val) => parseInt(val, 10))
		.default("300"),
	DATABASE_DIALECT: z
	...
	OBSERVABILITY_NEW_RELIC_LICENSE_KEY: z
		.string()
		.default("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"),
	OBSERVABILITY_NEW_RELIC_APP_LOG_FORWARDING_ENABLED: z
		.string()
		.default("true"),
	OBSERVABILITY_NEW_RELIC_DISTRIBUTED_TRACING_ENABLED: z
		.string()
		.default("true"),
